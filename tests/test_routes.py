...
def test_get_all_books_with_title_query_matching_none(client, two_saved_books):
    # Act
    query_param = {'title': 'Cloud Book'}
    response = client.get("/books", query_string = query_param)
    response_body = response.get_json()

    # Assert
    assert response.status_code == 200
    assert response_body == {"message":"book Cloud Book not found"}

def test_get_all_books_with_title_query_matching_one(client, two_saved_books):
    # Act
    query_param = {'title': 'Ocean Book'}
    response = client.get("/books", query_string = query_param)
    response_body = response.get_json()
...
def test_create_one_book_with_duplicate_keys(client, two_saved_books):
    # Arrange
    test_data = {
        "extra": "some stuff",
        "title": "New Book",
    ...
    # Assert
    assert response.status_code == 201
    assert response_body == "Book Dup Book successfully created"
